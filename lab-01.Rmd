...
```
#There are 348 Attractions in the Data Set
### Exercise a.
...
Not Available and it is because there are values in the 2021 variable that are NA.
...
There are very large individual values at the extreme end.
...
visitors_with_nations %>% 
  ...
  mutate(admission_free_percent = n / total * 100)
...
```{r Ex_n}
visitors_with_nations %>% 
  mutate(percentage_change_admissions = n_2022 / n_2021 * 100) %>% 
  filter(nation == "Scotland") %>% 
  group_by(admission) %>% 
  summarise(mean_percentage_change = mean(percentage_change_admissions, 
                                          na.rm = TRUE)) %>% 
